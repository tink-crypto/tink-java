...
public class WrappedChunkedMacTest {
  ...
  private static ChunkedMac primitiveFactory(KeysetHandleInterface.Entry entry)
      throws GeneralSecurityException {
    if (entry.getKey() instanceof HmacKey) {
      return new ChunkedHmacImpl((HmacKey) entry.getKey());
    }
    if (entry.getKey() instanceof AesCmacKey) {
      return ChunkedAesCmacImpl.create((AesCmacKey) entry.getKey());
    }
    throw new GeneralSecurityException(
        "Cannot create ChunkedMac for key " + entry.getKey().getClass());
  }
  ...
  public void testComputeVerifyMac_works() throws Exception {
    ...
    ChunkedMac mac =
        WrappedChunkedMac.create(smallKeysetHandle, WrappedChunkedMacTest::primitiveFactory);
  ...
  public void testComputeVerifyMac_throwsOnWrongKey() throws Exception {
    ...
    ChunkedMac mac0 =
        WrappedChunkedMac.create(computeKeysetHandle, WrappedChunkedMacTest::primitiveFactory);
    ...
    plaintext.rewind();
    ChunkedMac mac1 =
        WrappedChunkedMac.create(verifyKeysetHandle, WrappedChunkedMacTest::primitiveFactory);
  ...
  public void testComputeMac_usesPrimaryKey() throws Exception {
    ...
    ChunkedMac mac =
        WrappedChunkedMac.create(keysetHandle, WrappedChunkedMacTest::primitiveFactory);
    ChunkedMacComputation macComputation = mac.createComputation();
    macComputation.update(plaintext);
    byte[] tag = macComputation.computeMac();
    ...
    ChunkedMac primaryMac =
        WrappedChunkedMac.create(keysetHandlePrimary, WrappedChunkedMacTest::primitiveFactory);
  ...
  public void testComputeVerifyMac_manyKeysWork() throws Exception {
    ...
    ChunkedMac mac =
        WrappedChunkedMac.create(assortedKeysetHandle, WrappedChunkedMacTest::primitiveFactory);
    ChunkedMacComputation macComputation = mac.createComputation();
    macComputation.update(plaintext);
    byte[] tag = macComputation.computeMac();
  ...
  public void testVerifyMac_shiftedPrimaryWithManyKeysWorks() throws Exception {
    ...
    ChunkedMac mac0 =
        WrappedChunkedMac.create(assortedKeysetHandle0, WrappedChunkedMacTest::primitiveFactory);
    ChunkedMacComputation macComputation = mac0.createComputation();
    macComputation.update(plaintext);
    byte[] tag = macComputation.computeMac();
    ...
    plaintext.rewind();
    ChunkedMac mac1 =
        WrappedChunkedMac.create(assortedKeysetHandle1, WrappedChunkedMacTest::primitiveFactory);
    ChunkedMacVerification macVerification = mac1.createVerification(tag);
    macVerification.update(plaintext);
  ...
  public void disabledKey_isIgnored() throws Exception {
    ...
    ChunkedMacComputation tinkKey0PrimaryMac =
        WrappedChunkedMac.create(
                KeysetHandle.newBuilder()
                    .addEntry(KeysetHandle.importKey(tinkKey0).makePrimary())
                    .build(),
                WrappedChunkedMacTest::primitiveFactory)
            .createComputation();
