...
/**
 * Unit tests for {@link HmacPrf}. Note that this used to be a Mac primitive, so all these tests
 * first convert the Prf to a Mac.
 */
...
public class HmacPrfTest {
  ...
  private static Prf createHmacPrf(String algName, byte[] rawKeyBytes)
    ...
    switch (algName) {
      ...
      case "HMACSHA512":
        ...
        break;
      default:
        throw new IllegalArgumentException("Unknown algorithm: " + algName);
    }
    HmacPrfKey key =
        HmacPrfKey.builder()
            .setKeyBytes(SecretBytes.copyFrom(rawKeyBytes, InsecureSecretKeyAccess.get()))
            .setParameters(
  ...
  public void testMacTestVectors() throws Exception {
    ...
    for (MacTestVector t : hmacTestVectors) {
      Mac mac = new PrfMac(createHmacPrf(t.algName, t.key), t.tag.length);
  ...
  public void testPrfUniformity() throws GeneralSecurityException {
    ...
    for (MacTestVector t : hmacTestVectors) {
      Prf prf = createHmacPrf(t.algName, t.key);
  ...
  public void testTagTruncation() throws Exception {
    ...
    for (MacTestVector t : hmacTestVectors) {
      Mac mac = new PrfMac(createHmacPrf(t.algName, Random.randBytes(t.key.length)), t.tag.length);
      for (int j = 1; j < t.tag.length; j++) {
        byte[] modifiedTag = Arrays.copyOf(t.tag, t.tag.length - j);
        assertThrows(GeneralSecurityException.class, () -> mac.verifyMac(modifiedTag, t.message));
      }
  ...
  public void testThrowExceptionIfKeySizeIsTooSmall() throws Exception {
    ...
    assertThrows(
        ...
        () -> new PrfMac(createHmacPrf("HMACSHA1", Random.randBytes(15)), 16));
  }
  ...
  private static void testThrowExceptionIfTagSizeIsTooSmall(String algoName) throws Exception {
    for (int i = 0; i < MIN_TAG_SIZE_IN_BYTES; i++) {
      try {
        new PrfMac(createHmacPrf(algoName, Random.randBytes(16)), i);
  ...
  private static void testPrfNoExceptionIfTagSizeIsTooSmall(String algoName) throws Exception {
    for (int i = 0; i < MIN_TAG_SIZE_IN_BYTES; i++) {
      Object unused = createHmacPrf(algoName, Random.randBytes(16)).compute(new byte[100], i);
    }
  }
  ...
  private static void testThrowExceptionIfTagSizeIsTooLarge(String algoName, int tagSize)
      throws Exception {
    try {
      new PrfMac(createHmacPrf(algoName, Random.randBytes(16)), tagSize);
      fail("Expected InvalidAlgorithmParameterException");
    } catch (InvalidAlgorithmParameterException ex) {
      // expected.
    }
  ...
  public void testPrfThrowsExceptionIfTagSizeIsTooLarge(String algoName, int tagSize)
      throws Exception {
    try {
      Prf r = createHmacPrf(algoName, Random.randBytes(16));
  ...
  public void testFailIfFipsModuleNotAvailable() throws Exception {
    ...
    assertThrows(
        GeneralSecurityException.class, () -> createHmacPrf("HMACSHA256", Random.randBytes(16)));
  }
}
