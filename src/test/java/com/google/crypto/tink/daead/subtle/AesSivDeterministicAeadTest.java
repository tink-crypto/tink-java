...
/** Tests for {@link AesSivDeterministicAead} */
@RunWith(Theories.class)
@LowLevelCryptoCaller
public class AesSivDeterministicAeadTest {

  private static final int KEY_SIZE_IN_BYTES = 64;

  private static DeterministicAeads createAesSivDeterministicAead(byte[] rawKeyBytes)
      throws GeneralSecurityException {
    AesSivParameters parameters =
        AesSivParameters.builder()
            .setKeySizeBytes(rawKeyBytes.length)
            .setVariant(AesSivParameters.Variant.NO_PREFIX)
            .build();
    SecretBytes keyBytes = SecretBytes.copyFrom(rawKeyBytes, InsecureSecretKeyAccess.get());
  ...
  public void create_nullKey_throws() throws Exception {
    assertThrows(NullPointerException.class, () -> AesSivDeterministicAead.create(null));
  }
  ...
  public void testWycheproofVectors() throws Exception {
    ...
    JsonObject json =
        WycheproofTestUtil.readJson("third_party/wycheproof/testvectors_v1/aes_siv_cmac_test.json");
    ...
    for (int i = 0; i < testGroups.size(); i++) {
      ...
      for (int j = 0; j < tests.size(); j++) {
        ...
        if (keySizeInBits == 384) {
          ...
          assertThrows(
              InvalidKeyException.class, () -> createAesSivDeterministicAead(key));
          continue;
        }
        ...
        DeterministicAead daead = createAesSivDeterministicAead(key);
  ...
  public void testWycheproofVectors_createNoPrefix() throws Exception {
    ...
    for (int i = 0; i < testGroups.size(); i++) {
      ...
      for (int j = 0; j < tests.size(); j++) {
        ...
        if (keySizeInBits == 384) {
          // These key sizes are currently not supported.
          assertThrows(InvalidKeyException.class, () -> AesSivDeterministicAead.create(aesSivKey));
          continue;
        }
        DeterministicAead daead = AesSivDeterministicAead.create(aesSivKey);
        if (result.equals("valid")) {
          byte[] ciphertext = daead.encryptDeterministically(msg, aad);
          assertEquals(tcId, Hex.encode(ct), Hex.encode(ciphertext));
          byte[] plaintext = daead.decryptDeterministically(ct, aad);
  ...
  public void testEncryptDecryptWithEmptyPlaintext() throws GeneralSecurityException {
    Assume.assumeFalse(TinkFips.useOnlyFips());
    DeterministicAead dead = createAesSivDeterministicAead(Random.randBytes(KEY_SIZE_IN_BYTES));
    for (int triesPlaintext = 0; triesPlaintext < 100; triesPlaintext++) {
      ...
      assertThat(ciphertext).hasLength(16);
  ...
  public void testModifiedCiphertext() throws GeneralSecurityException {
    ...
    DeterministicAead crypter = createAesSivDeterministicAead(key);
  ...
  public void tooManyAssociatedDatas_throws() throws GeneralSecurityException {
    ...
    DeterministicAeads crypter = createAesSivDeterministicAead(key);
  ...
  public void testCreateForEncryptConstructorForDecrypt_noPrefix() throws GeneralSecurityException {
    ...
    DeterministicAeads daead = AesSivDeterministicAead.create(key);
  ...
  public void testCreateForEncryptConstructorForDecrypt_tinkPrefix()
    ...
    AesSivKey key =
        AesSivKey.builder()
            ...
            .build();
    DeterministicAead daead = AesSivDeterministicAead.create(key);
  ...
  public void testKeySize48_throws() throws GeneralSecurityException {
    ...
    AesSivKey key = AesSivKey.builder().setParameters(parameters).setKeyBytes(keyBytes).build();
    assertThrows(GeneralSecurityException.class, () -> AesSivDeterministicAead.create(key));
  }
  ...
  public void testvector_works(@FromDataPoints("testCases") TestVector test)
    ...
    DeterministicAeads aesSiv = createAesSivDeterministicAead(test.key);
