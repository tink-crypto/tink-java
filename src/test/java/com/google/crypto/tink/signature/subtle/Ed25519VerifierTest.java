...
public final class Ed25519VerifierTest {
    ...
    for (int i = 0; i < testGroups.size(); i++) {
      ...
      for (int j = 0; j < tests.size(); j++) {
        ...
        PublicKeyVerify verifier =
            Ed25519Verifier.create(
                Ed25519PublicKey.create(com.google.crypto.tink.util.Bytes.copyFrom(publicKey)));
        try {
  ...
  /**
   * Tests that the verifier can verify the signature for the message and key in the test vector.
   */
  @Theory
  public void verifySignatureInTestVector_works(
    ...
    PublicKeyVerify verifier = Ed25519Verifier.create(key.getPublicKey());
