...
/** Tests for {@link WrappedHybridDecrypt}. */
@RunWith(Theories.class)
public class WrappedHybridDecryptTest {
  @Before
  ...
  public void decryptNoPrefix_works() throws Exception {
    ...
    HybridDecrypt decrypter =
        WrappedHybridDecrypt.create(handle, WrappedHybridDecryptTest::primitiveFactory);
  ...
  public void decrypt_rawCiphertextLookingLikeTinkCiphertext_createTest() throws Exception {
    ...
    HybridDecrypt keysetHandleDecrypt =
        WrappedHybridDecrypt.create(handle, WrappedHybridDecryptTest::primitiveFactory);
  ...
  private static HybridDecrypt primitiveFactory(KeysetHandleInterface.Entry entry)
      throws GeneralSecurityException {
    if (entry.getKey() instanceof HpkePrivateKey) {
      return HpkeDecrypt.create((HpkePrivateKey) entry.getKey());
    }
    if (entry.getKey() instanceof EciesPrivateKey) {
      return EciesAeadHkdfHybridDecrypt.create((EciesPrivateKey) entry.getKey());
    }
    throw new GeneralSecurityException(
        "Cannot create HybridDecrypt for key " + entry.getKey().getClass());
  }
}
