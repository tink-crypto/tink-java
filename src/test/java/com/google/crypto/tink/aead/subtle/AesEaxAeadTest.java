...
/** Unit tests for {@link AesEaxAead}. */
@RunWith(Theories.class)
@LowLevelCryptoCaller
public final class AesEaxAeadTest {
  private static final int KEY_SIZE = 16;
  private static final int IV_SIZE = 16;
  private Integer[] validKeySizesInBytes;
  private Integer[] validIvSizesInBytes;

  private static Aead getAead(byte[] keyBytes, int ivSize) throws GeneralSecurityException {
  ...
  public void create_nullKey_throws() throws Exception {
    assertThrows(NullPointerException.class, () -> AesEaxAead.create(null));
  }
          ...
          assertThrows(
              ...
              () ->
                  AesEaxParameters.builder()
                      ...
                      .setIvSizeBytes(iv.length)
                      .setTagSizeBytes(tag.length)
                      .setVariant(AesEaxParameters.Variant.NO_PREFIX)
                      .build());
          continue;
        }
        if (tag.length != 16) {
          ...
          AesEaxKey eaxKey =
              AesEaxKey.builder()
                  ...
                  .build();
          assertThrows(GeneralSecurityException.class, () -> AesEaxAead.create(eaxKey));
          continue;
        }
        if (keySizeInBits == 192) {
          // 192-bit keys are not supported for execution.
          AesEaxParameters parameters =
              AesEaxParameters.builder()
                  .setKeySizeBytes(key.length)
                  .setIvSizeBytes(iv.length)
        ...
        try {
          ...
          AesEaxKey eaxKey =
              AesEaxKey.builder()
                  ...
                  .setKeyBytes(SecretBytes.copyFrom(key, InsecureSecretKeyAccess.get()))
                  .build();
          Aead eax = AesEaxAead.create(eaxKey);
  ...
  public void testEncryptDecrypt() throws Exception {
    ...
    for (Integer keySizeInBytes : validKeySizesInBytes) {
      for (Integer ivSizeInBytes : validIvSizesInBytes) {
        ...
        Aead eax = getAead(key, ivSizeInBytes);
  ...
  public void testNullPlaintextOrCiphertext() throws Exception {
    ...
    Aead eax = getAead(Random.randBytes(KEY_SIZE), IV_SIZE);
    byte[] aad = new byte[] {1, 2, 3};
    assertThrows(
  ...
  public void testVector_decrypt_works(@FromDataPoints("testVectors") TestVector vector)
    ...
    Aead eax = getAead(Hex.decode(vector.hexKey), Hex.decode(vector.hexNonce).length);
  ...
  public void testWithAesEaxKey_noPrefix_works() throws Exception {
    ...
    AesEaxKey key =
        AesEaxKey.builder()
            ...
            .setKeyBytes(SecretBytes.copyFrom(keyBytes, InsecureSecretKeyAccess.get()))
            .build();
    Aead aead = AesEaxAead.create(key);
