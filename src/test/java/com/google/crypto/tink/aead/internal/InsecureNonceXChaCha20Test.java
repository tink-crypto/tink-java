...
public class InsecureNonceXChaCha20Test {
  public InsecureNonceXChaCha20 createInstance(final byte[] key) throws InvalidKeyException {
    return new InsecureNonceXChaCha20(key, 0 /* initialCounter */);
  }
  ...
  public void testEncryptDecrypt() throws Exception {
    for (int i = 0; i < 64; i++) {
      ...
      InsecureNonceXChaCha20 cipher = createInstance(key);
      for (int j = 0; j < 64; j++) {
  ...
  @Test
  public void testDecryptThrowsGeneralSecurityExpWithInvalidNonceSize() throws Exception {
    InsecureNonceXChaCha20 cipher = createInstance(Random.randBytes(32));
    byte[] nonce = Random.randBytes(25);
    byte[] input = Random.randBytes(16);
    GeneralSecurityException e =
        assertThrows(GeneralSecurityException.class, () -> cipher.encrypt(nonce, input));
    assertThat(e).hasMessageThat().contains("The nonce length (in bytes) must be 24");
  ...
  public void testXChaCha20TestVectors() throws Exception {
    for (XChaCha20TestVector test : xChaCha20TestVectors) {
      InsecureNonceXChaCha20 cipher = new InsecureNonceXChaCha20(test.key, 0 /* initialCounter */);
      assertThat(cipher.encrypt(test.nonce, test.plaintext)).isEqualTo(test.ciphertext);
      assertThat(cipher.decrypt(test.nonce, test.ciphertext)).isEqualTo(test.plaintext);
    }
  }
}
