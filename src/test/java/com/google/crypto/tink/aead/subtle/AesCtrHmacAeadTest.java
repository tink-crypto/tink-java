...
/** Unit tests for {@link AesCtrHmacAead}. */
@RunWith(Theories.class)
@LowLevelCryptoCaller
public final class AesCtrHmacAeadTest {
  ...
  public void create_nullKey_throws() throws Exception {
    assertThrows(NullPointerException.class, () -> AesCtrHmacAead.create(null));
  }
  ...
  public void create_RFCVectors_works(@FromDataPoints("rfcTestVectors") RFCTestVector t)
    ...
    Aead aead = AesCtrHmacAead.create(key);
  ...
  private Aead getAead(byte[] hmacKey, byte[] encKey, int ivSize, int tagLength, String macAlg)
    ...
    switch (macAlg) {
      ...
      case "SHA512":
        ...
        break;
      default:
        throw new InvalidAlgorithmParameterException("unexpected hash type: " + macAlg);
    }
    AesCtrHmacAeadParameters parameters =
        AesCtrHmacAeadParameters.builder()
            .setAesKeySizeBytes(encKey.length)
            .setHmacKeySizeBytes(hmacKey.length)
            .setTagSizeBytes(tagLength)
            .setHashType(hashType)
    ...
    AesCtrHmacAeadKey key =
        AesCtrHmacAeadKey.builder()
            .setAesKeyBytes(SecretBytes.copyFrom(encKey, InsecureSecretKeyAccess.get()))
            .setHmacKeyBytes(SecretBytes.copyFrom(hmacKey, InsecureSecretKeyAccess.get()))
            .setParameters(parameters)
            .build();
    return AesCtrHmacAead.create(key);
  }
}
