...
public class SlhDsaPublicKey extends SignaturePublicKey {
  ...
  /** Builder for SlhDsaPublicKey. */
  public static class Builder {
    @Nullable private SlhDsaParameters parameters = null;
    ...
    private Bytes getOutputPrefix() {
      ...
      if (parameters.getVariant() == SlhDsaParameters.Variant.NO_PREFIX) {
        return OutputPrefixUtil.EMPTY_PREFIX;
      }
      if (parameters.getVariant() == SlhDsaParameters.Variant.TINK) {
        return OutputPrefixUtil.getTinkOutputPrefix(idRequirement);
      }
      throw new IllegalStateException(
          "Unknown SlhDsaParameters.Variant: " + parameters.getVariant());
    }
    ...
    public SlhDsaPublicKey build() throws GeneralSecurityException {
      ...
      if (parameters.getVariant() == SlhDsaParameters.Variant.NO_PREFIX && idRequirement != null) {
        throw new GeneralSecurityException(
            "IdRequirement must be null for variant NO_PREFIX");
      }
      if (parameters.getVariant() == SlhDsaParameters.Variant.TINK && idRequirement == null) {
      ...
      if (parameters.getHashType() != SlhDsaParameters.HashType.SHA2) {
        throw new GeneralSecurityException(
            "Unknown SLH-DSA hash type option "
                + parameters.getHashType()
                + "; only SHA2 is currently supported");
      }
      if (parameters.getPrivateKeySize() != SlhDsaParameters.SLH_DSA_128_PRIVATE_KEY_SIZE_BYTES) {
        throw new GeneralSecurityException(
            "Unknown SLH-DSA private key size "
                + parameters.getPrivateKeySize()
                + "; only security level 128 (private key size 64) is currently supported");
      }
      if (parameters.getSignatureType() != SlhDsaParameters.SignatureType.SMALL_SIGNATURE) {
        throw new GeneralSecurityException(
            "Unknown SLH-DSA signature type "
                + parameters.getSignatureType()
                + "; only \"S\" (SMALL_SIGNATURE) is currently supported");
      }
      if (serializedPublicKey.size() != SLH_DSA_SHA2_128S_PUBLIC_KEY_BYTES) {
        throw new GeneralSecurityException(
            "Incorrect public key size for SLH-DSA-SHA2-128S: should be "
      ...
      return new SlhDsaPublicKey(parameters, serializedPublicKey, outputPrefix, idRequirement);
    }
  }
