 ...
 *
 * <p>It uses the JCE implementation if available. If not, it uses Tink's subtle implementation.
 *
 * <p>On Android and on Java since JDK 11, the JCE implementation is available by default.
 */
@Immutable
final class X25519HpkeKem implements HpkeKem {
  ...
  X25519HpkeKem(HkdfHpkeKdf hkdf) {
    this.hkdf = hkdf;
    this.useJce = X25519Jce.isSupported();
  }
  ...
  HpkeKemEncapOutput encapsulateWithFixedEphemeralKey(
    ...
    byte[] dhSharedSecret;
    if (useJce) {
      dhSharedSecret = X25519Jce.computeSharedSecret(ephemeralPrivateKey, recipientPublicKey);
    } else {
      dhSharedSecret = X25519.computeSharedSecret(ephemeralPrivateKey, recipientPublicKey);
    }
  ...
  public HpkeKemEncapOutput encapsulate(byte[] recipientPublicKey) throws GeneralSecurityException {
    ...
    if (useJce) {
      ...
      ephemeralPrivateKey = X25519.generatePrivateKey();
      ephemeralPublicKey = X25519.publicFromPrivate(ephemeralPrivateKey);
    }
  ...
  HpkeKemEncapOutput authEncapsulateWithFixedEphemeralKey(
    ...
    if (useJce) {
      dhSharedSecret =
          Bytes.concat(
              X25519Jce.computeSharedSecret(ephemeralPrivateKey, recipientPublicKey),
              X25519Jce.computeSharedSecret(
  ...
  public byte[] authDecapsulate(
    ...
    if (useJce) {
      dhSharedSecret =
          Bytes.concat(
              X25519Jce.computeSharedSecret(privateKey, encapsulatedKey),
              X25519Jce.computeSharedSecret(privateKey, senderPublicKey));
    } else {
      dhSharedSecret =
          Bytes.concat(
